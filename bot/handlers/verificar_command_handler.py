...
async def verificar(update: Update, context: CallbackContext) -> None:
    ...
    if not message_is_on_group(CHAT_ID):
        ...
        bot_logger.info(f"Usuário {update.effective_user.username} tentou utilizar o comando /apelidar fora de um grupo autorizado | Chat: {format_chat_object(update)}")
        return
    ...
    if not await user_is_group_admin(update):
        ...
        bot_logger.info(f"Usuário {update.effective_user.username} utilizou, sem permissão, o comando /apelidar | Chat: {format_chat_object(update)}")
        return
    ...
    if len(context.args) < 2:
        ...
        bot_logger.info(f"Usuário {update.effective_user.username} utilizou o comando /apelidar sem todos argumentos ({update.message.text}) | Chat: {format_chat_object(update)}")
        return
    ...
    if not username.startswith("@"):
        ...
        bot_logger.info(f"Usuário {update.effective_user.username} utilizou o comando /apelidar sem @ no username ({update.message.text}) | Chat: {format_chat_object(update)}")
        return
